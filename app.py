...
def read_rooms() -> list[dict[str, Any]]:
    return read_json(ROOMS_FILE)


def write_rooms(payload: list[dict[str, Any]]) -> None:
    write_json(ROOMS_FILE, payload)


def room_peer_user_id(room: dict[str, Any], user_id: str) -> str:
    return room.get("guest_user_id") if room.get("host_user_id") == user_id else room.get("host_user_id")
...
def get_room(room_id: str) -> dict[str, Any] | None:
    return next((room for room in read_rooms() if room["id"] == room_id), None)


def send_ws_json(connection: ServerConnection, message: dict[str, Any]) -> None:
    connection.send(json.dumps(message, ensure_ascii=False))


def send_ws_to_user(room_id: str, user_id: str, message: dict[str, Any]) -> None:
    with LOCK:
        room_connections = ROOM_SOCKET_CONNECTIONS.get(room_id, {})
        connection = room_connections.get(user_id)
    if connection is None:
        return
    try:
        send_ws_json(connection, message)
    except Exception:
        pass


def broadcast_ws_to_room(room_id: str, message: dict[str, Any], *, exclude_user_id: str | None = None) -> None:
    with LOCK:
        targets = [
            (user_id, connection)
            for user_id, connection in ROOM_SOCKET_CONNECTIONS.get(room_id, {}).items()
...
def register_room_socket(room_id: str, user_id: str, connection: ServerConnection) -> None:
    with LOCK:
        ROOM_SOCKET_CONNECTIONS.setdefault(room_id, {})[user_id] = connection


def unregister_room_socket(room_id: str, user_id: str, connection: ServerConnection) -> None:
    with LOCK:
        room_connections = ROOM_SOCKET_CONNECTIONS.get(room_id, {})
        if room_connections.get(user_id) is connection:
            room_connections.pop(user_id, None)
        if not room_connections and room_id in ROOM_SOCKET_CONNECTIONS:
            ROOM_SOCKET_CONNECTIONS.pop(room_id, None)


def notify_peer_joined(room_id: str, user_id: str) -> None:
    room = get_room(room_id)
    if room is None:
        return
    peer_user_id = room_peer_user_id(room, user_id)
    if not peer_user_id:
        return
    joined_message = build_ws_message(
        "peer_joined",
        room_id,
        from_user_id=user_id,
        to_user_id=peer_user_id,
        payload={"peerUserId": user_id},
    )
    send_ws_to_user(room_id, peer_user_id, joined_message)
    reverse_message = build_ws_message(
        "peer_joined",
        room_id,
        from_user_id=peer_user_id,
        to_user_id=user_id,
        payload={"peerUserId": peer_user_id},
    )
    send_ws_to_user(room_id, user_id, reverse_message)


def handle_ws_message(room_id: str, user_id: str, message: dict[str, Any]) -> None:
    message_type = str(message.get("type", "") or "").strip()
    target_user_id = str(message.get("toUserId", "") or "").strip() or None
    payload = message.get("payload", {})
    if not message_type:
        return
    if message_type in {"offer", "answer", "ice", "sticker_sent", "frame_uploaded"} and target_user_id:
        outbound = build_ws_message(
            message_type,
            room_id,
            from_user_id=user_id,
        ...
        send_ws_to_user(room_id, target_user_id, outbound)


def room_ws_process_request(connection: ServerConnection, ws_request: Request) -> None:
    connection.tapshow_path = ws_request.path
    return None


def room_ws_handler(connection: ServerConnection) -> None:
    parsed = urlparse(getattr(connection, "tapshow_path", "/"))
        ...
        try:
            send_ws_json(
                ...
                build_ws_message(
                    ...
                    room_id or "",
                    from_user_id="server",
                    to_user_id=user_id or None,
                    payload={"code": "INVALID_WS_PARAMS", "message": "roomId and userId are required"},
                ),
            )
        finally:
            connection.close()
    ...
    register_room_socket(room_id, user_id, connection)
    peer_user_id = room_peer_user_id(room, user_id)
    send_ws_json(
        connection,
        build_ws_message(
            ...
            payload={
                "role": role,
                "peerPresent": bool(peer_user_id),
                "peerUserId": peer_user_id,
            },
        ),
    )
    if peer_user_id:
        notify_peer_joined(room_id, user_id)
            ...
            try:
                ...
                send_ws_json(
                    ...
                    build_ws_message(
                        ...
                        from_user_id="server",
                        to_user_id=user_id,
                        payload={"code": "INVALID_JSON", "message": "Invalid websocket message"},
                    ),
                )
                continue
            if isinstance(message, dict):
                handle_ws_message(room_id, user_id, message)
    except ConnectionClosed:
        pass
    finally:
        unregister_room_socket(room_id, user_id, connection)
        if peer_user_id:
            send_ws_to_user(
                room_id,
                peer_user_id,
...
def run_ws_signal_server(host: str = "0.0.0.0", port: int = WS_SIGNAL_PORT, ssl_context: ssl.SSLContext | None = None) -> None:
    ...
    with serve(
        ...
        host,
        port,
        process_request=room_ws_process_request,
        ssl=ssl_context,
    ) as server:
...
def should_retry_without_ssl_verification(exc: error.URLError) -> bool:
    reason = getattr(exc, "reason", exc)
    return "CERTIFICATE_VERIFY_FAILED" in str(reason)


def open_ark_request(req: request.Request, timeout: int = 180) -> dict[str, Any]:
    attempts = ({}, {"context": ssl._create_unverified_context()})
    for index, kwargs in enumerate(attempts):
        try:
            with request.urlopen(req, timeout=timeout, **kwargs) as response:
                try:
            ...
            if index == 0 and should_retry_without_ssl_verification(exc):
                continue
            raise


def download_image_as_data_url(image_url: str, timeout: int = 180) -> str:
        ...
        try:
            with request.urlopen(req, timeout=timeout, **kwargs) as response:
                try:
                    ...
                    content = exc.partial
                mime = response.headers.get_content_type() if response.headers else "image/png"
...
def call_ark_image_generation(source_images: str | list[str], target_count: int = 3) -> dict[str, Any]:
    ...
    for base_url in base_urls:
        for request_url in normalize_ark_request_urls(base_url):
            ...
            try:
                data = open_ark_request(req, timeout=180)
            ...
            for item in data.get("data", []):
                ...
                if image_b64:
                    ...
                    try:
                        normalized = download_image_as_data_url(image_url)
                    except Exception:
...
def postprocess_sticker(sticker: dict[str, Any]) -> dict[str, Any]:
    ...
    for source_url in source_urls:
        try:
            normalized_source = download_image_as_data_url(source_url) if source_url.startswith("http") else source_url
            processed_urls.append(remove_background_from_data_url(normalized_source))
        except Exception:
...
def run_server(host: str = "0.0.0.0", port: int = 8000, https_port: int = 8443) -> None:
    ...
    background_threads.append(
        threading.Thread(
            target=run_ws_signal_server,
            kwargs={"host": host, "port": WS_SIGNAL_PORT},
            daemon=True,
            name="tapshow-ws-http",
        )
    )
    ...
    if HTTPS_CERT_FILE.exists() and HTTPS_KEY_FILE.exists():
        ...
        background_threads.append(
            threading.Thread(
                target=run_ws_signal_server,
                kwargs={"host": host, "port": WSS_SIGNAL_PORT, "ssl_context": ssl_context},
                daemon=True,
                name="tapshow-ws-https",
            )
        )
    ...
    print(f"TapShow WS signal server running at ws://{host}:{WS_SIGNAL_PORT}")
    if len(servers) > 1:
